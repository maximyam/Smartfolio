...
-----------------------------
This module provides functions to adjust the quantities of equities in a portfolio to maximize its alpha while keeping the total value unchanged.

Functions:
1. `maximize_portfolio_alpha`: Adjusts quantities to maximize the portfolio's alpha.
...
Outputs:
- A new list of equities with adjusted quantities that maximize the portfolio alpha while maintaining the portfolio value.
...
def maximize_portfolio_alpha(equities, benchmark_return, risk_free_rate):
    n = len(equities)

    # Coefficients for the objective function (we want to maximize alpha hence minimize negative alpha)
    ...
    # Linear programming to maximize portfolio alpha (minimize negative alpha)
...
optimized_equities = maximize_portfolio_alpha(equities, benchmark_return, risk_free_rate)
